...
setMethod(
    "samc",
    signature(data = "SpatRaster", absorption = "SpatRaster", fidelity = "SpatRaster", model = "list"),
    function(data, absorption, fidelity, model, options = NULL) {
        ...
        samc_obj = methods::new("samc",
                                data = methods::new("samc_data", f = new("dgCMatrix"), t_abs = numeric(0)),
                                conv_cache = NULL,
                                model = model,
                                source = "SpatRaster",
        ...
        if (options$method %in% c("direct", "iter")) {
            if (model$name %in% c("RW", "SSF")) {
                ...
                warning("CRW support is currently experimental")
        ...
        samc_obj@map[map_cells] = seq_along(map_cells)
...
setMethod(
    "samc",
    signature(data = "RasterLayer", absorption = "RasterLayer", fidelity = "RasterLayer", model = "list"),
    function(data, absorption, fidelity, model, options = NULL) {
...
setMethod(
    "samc",
    signature(data = "SpatRaster", absorption = "SpatRaster", fidelity = "missing", model = "list"),
...
setMethod(
    "samc",
    signature(data = "RasterLayer", absorption = "RasterLayer", fidelity = "missing", model = "list"),
    function(data, absorption, model, options = NULL) {
...
setMethod(
    "samc",
    signature(data = "matrix", absorption = "matrix", fidelity = "matrix", model = "list"),
    function(data, absorption, fidelity, model, options = NULL) {
...
setMethod(
    "samc",
    signature(data = "matrix", absorption = "matrix", fidelity = "missing", model = "list"),
    function(data, absorption, model, options = NULL) {
...
setMethod(
    "samc",
    signature(data = "dgCMatrix", absorption = "missing", fidelity = "missing", model = "missing"),
...
setMethod(
    "samc",
    signature(data = "matrix", absorption = "missing", fidelity = "missing", model = "missing"),
    function(data, options = NULL) {
