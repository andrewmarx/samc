...
First, we the coin flip probabilities are defined using variables, which allows us to easily change the matrix to simulate a biased coin.
...
In fact, for the original question, it's possible to collapse some of these states and simplify the matrix. For example, $HTT$ would also require a minimum of 3 flips to reach, so it could also be used as a starting point to answer the original question. However, the current construction allows different questions to be asked about the model. It could even be extended with more possible states (e.g., combinations of four coin flips).
