...
To run metrics using the CRW, a single origin and direction value must be supplied in a 1x2 matrix (this may change in the next version of the package). Support for multiple `origin` values, `init` maps, and `dest` values is being explored for future releases.
