...
Both SAMC and circuit theory use Markov chain theory to model movement based on variation in the landscape coded as resistance values. However, SAMC extends these capabilities by utilizing absorbing Markov chains, which allow it to model a variety of short- and long-term metrics. SAMC also allows for the explicit incorporation of multiple absorption states into the model. These states can represent any effect that might permanently stop movement in the landscape, such as:
...
Throughout this document, circuit theory should be understood as a special case of a broader random-walk framework. The goal here is not to replace circuit theory, but to clarify how its core ideas fit within the more general SAMC formulation.
