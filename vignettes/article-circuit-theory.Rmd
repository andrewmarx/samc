...
```{r}
reg_net_samc = visitation_net(samc_obj, origin = origin_cell)
reg_samc_ras = map(samc_obj, reg_net_samc)
plot(reg_samc_ras,  main = "Net Movement Flow (samc with absorption)", col = viridis(256))
```

Overall, incorporating absorption more broadly has reduced the net movement flow because a portion of the individuals is now being removed before reaching the destination. More importantly, the incorporation of absorption now means the destination is not guaranteed to be reached, leading to a dramatic drop in the net movement at the destination cell. We can subtract the non-absorption model from the absorption model to see how much net movement flow has decreased.

```{r}
net_diff = reg_samc_ras - net_gdist_ras

plot(net_diff, main = "Effect of Absorption on Net Movement Flow", col = viridis(256))

# Bin for easier interpretation
net_diff_binned = classify(net_diff, rcl = c(-1, -0.1, -0.01, 0, 0.01, 0.1, 1), right = FALSE)
plot(net_diff_binned, main = "Effect of Absorption on Net Movement Flow (Binned)", col = viridis(256))
```

Overall, we see that flow has increased on the bottom half and decreased on the top half. First, on the right path, there is a noticeable increase in flow where it crosses the absorption strip. This is because once movement reaches it, the increase in absorption causes some of the movement to stop and fail to leave. Then above this, movement flow decreases because the absorption barrier is limiting movement in from below. Third, we see a decrease in slight decrease in movement flow on the left path, despite an opening in the absorption barrier. That reduction in flow is probably largely attributable to the flow attempting to cross the barrier on either side of the path. Finally, we can see that the presence of the absorption strip has substantially decreased the net movement flow reaching the destination. We can look at the values directly to see the reduction is over 98%:
