...
There are multiple approaches to calculating the results of samc
...
goals, and computing resources.

## Methods

### Linear Algebra Solvers

Linear algebra solvers take the transition matrix and calculate the
...
- Caching behavior makes reruns of long-term metric calculations with
  different inputs nearly instantaneous.
...
The main benefit of the direct linear algebra behavior is that its
...
The direct solver is used by default but can be manually specified
during the creation of the samc object:

    samc(..., options = list(method = "direct"))

It can also be set after the samc object has been created as long as
long as the samc object is built with a transition matrix:
...
- Technically produces an approximate result, but the difference is not
  noticeable (within machine precision)

The main benefit of the iterative linear algebra solver is that it can
be used for any metric while being memory efficient. The downside is
that it will generally be the slowest option for real-world data.

The iterative solver can be manually specified during the creation of
the samc object:
...
The convolution method is an entirely different approach from using
...
creation like with the linear algebra solvers.

The convolution algorithm can only calculate results for metrics one
...
diminishing returns after two threads.

When calculating all metrics, the algorithm is very memory efficient,
...
the calculations are moderately faster than the linear algebra approach.

To use the convolution method, it must be specified during the creation
of the samc object:
...
|:--------------------------------------------------------------------------------|:-------------------------------------------------------------|:-------|:----------|:------------|
| [`absorption()`](https://andrewmarx.github.io/samc/reference/absorption.md)     | A = F R                                                      | ✓      | ✓^2       |             |
|                                                                                 | \psi^T A                                                     | ✓      | ✓         | ✓^2         |
| [`cond_passage()`](https://andrewmarx.github.io/samc/reference/cond_passage.md) | \tilde{t} = \tilde{B}\_j^{-1}\tilde{F}\tilde{B}\_j{\cdot}1   | ✓      | ✓         |             |
| [`dispersal()`](https://andrewmarx.github.io/samc/reference/dispersal.md)       | \tilde{D}\_{jt}=({\sum}\_{n=0}^{t-1}\tilde{Q}^n)\tilde{q}\_j | ✓^1    | ✓^1       |             |
|                                                                                 | \psi^T\tilde{D}\_{jt}                                        | ✓^1    | ✓^1       |             |
|                                                                                 | D=(F-I)diag(F)^{-1}                                          | ✓      | ✓         |             |
|                                                                                 | \psi^TD                                                      | ✓      | ✓         |             |
| [`distribution()`](https://andrewmarx.github.io/samc/reference/distribution.md) | Q^t                                                          | ✓^1    | ✓^1       |             |
|                                                                                 | \psi^TQ^t                                                    | ✓^1    | ✓^1       | ✓           |
| [`mortality()`](https://andrewmarx.github.io/samc/reference/mortality.md)       | \tilde{B}\_t = (\sum\_{n=0}^{t-1} Q^n) \tilde{R}             | ✓^1    | ✓^1       |             |
|                                                                                 | \psi^T \tilde{B}\_t                                          | ✓^1    | ✓^1       | ✓           |
|                                                                                 | B = F \tilde{R}                                              | ✓      | ✓         |             |
|                                                                                 | \psi^T B                                                     | ✓      | ✓         | ✓^2         |
| [`survival()`](https://andrewmarx.github.io/samc/reference/survival.md)         | z=(I-Q)^{-1}{\cdot}1=F{\cdot}1                               | ✓      | ✓         |             |
|                                                                                 | {\psi}^Tz                                                    | ✓      | ✓         | ✓^2         |
| [`visitation()`](https://andrewmarx.github.io/samc/reference/visitation.md)     | \tilde{F}\_t = \sum\_{n=0}^{t-1} Q^n                         | ✓^1    | ✓^1       |             |
|                                                                                 | {\psi}^T \tilde{F}\_t                                        | ✓^1    | ✓^1       | ✓           |
|                                                                                 | F = (I-Q)^{-1}                                               | ✓      | ✓         |             |
|                                                                                 | {\psi}^T F                                                   | ✓      | ✓         | ✓^2         |

^1 Short-term metrics do not use linear algebra solvers. Instead, they
...
have the same performance characteristics as the long-term metrics.

^2 The number of iterations or steps to convergence for the long-term
