...
The third part of the series takes the perfect maze with only a single
solution and modifies it so that it now has more than one solution.
Afterward, all of the changes introduced in the series are combined into
a single complex example.
...
Part 3 reuses the code from [Part
1](https://andrewmarx.github.io/samc/articles/example-maze-part2.md) and
[Part
2](https://andrewmarx.github.io/samc/articles/example-maze-part2.md).

## Shortcut

To start, the original maze will be modified so that it has a new cell
bridging previously existing cells. The location was chosen so that it
effectively acts as a shortcut to the previous solution. To make things
more interesting, it will be treated as a secret passage by giving it a
high resistance value so that it is only rarely utilized. This is
effectively the same method used to model dead-end avoidance in Part 2,
but here the interpretation is different because of where the high
...
![](example-maze-part3_files/figure-html/unnamed-chunk-2-1.png)

Use `gdistance` to quickly verify the change in distance for the
shortest solution:
...
Since the structure of the resistance input has changed, the absorption
input also has to be changed so that it has the same structure:
...
With new resistance and absorption inputs, the samc object has to be
recreated:
...
An important thing to keep in mind: modifying where `NA` and non-`NA`
cells are located in the map means that previous results from
[`locate()`](https://andrewmarx.github.io/samc/reference/locate.md) are
no longer guaranteed to be valid. In this case, `start` is technically
the same, but `finish` is not and will return incorrect results, or in
special cases even lead to a crash.

With that, let’s begin exploring the different metrics and compare them
to the results from Part 1. Starting with
...
![](example-maze-part3_files/figure-html/unnamed-chunk-6-1.png)

There are two noticeable changes. First, the expected time to finish is
drastically decreased for the starting point:
...
The second change is the overall increase in expected time to finish
...
Part 2 showed that the inclusion of additional absorbing states changes
the relationship between
[`survival()`](https://andrewmarx.github.io/samc/reference/survival.md)
and
[`cond_passage()`](https://andrewmarx.github.io/samc/reference/cond_passage.md).
Since this example is back to a single absorbing state at the finish
point, this relationship is restored:
...
Part 2 also showed that including additional absorbing states affects
the probability of cells being visited. Modifying the maze to have
multiple routes also has significant ramifications:
...
![](example-maze-part3_files/figure-html/unnamed-chunk-9-1.png)

Notably, it is again no longer possible to discern the optimal route
through the maze by looking for cells with a probability of `1.0`. But,
like Part 2, a partial path can still be identified:
...
![](example-maze-part3_files/figure-html/unnamed-chunk-10-1.png)

In this case, the result contains the solution for the regions before
and after the loop created by the shortcut.

## Combined Example

So far, a variety of changes to the original maze have been
...
![](example-maze-part3_files/figure-html/unnamed-chunk-11-1.png)![](example-maze-part3_files/figure-html/unnamed-chunk-11-2.png)

There is an unusual artifact present in our new resistance raster: our
shortcut leads into a former dead end, and the dead end resistance value
will increase the probability of the individual turning around in the
shortcut even though, from the perspective of the shortcut, it is not a
dead end. However, when approaching from the bottom, the dead end still
exists if we assume the shortcut is hidden. This is a case where having
asymmetric transition probabilities would be a more ideal solution, but
for simplicity, we will leave things as they are.
...
Following the previous model changes, start by looking at the expected
time to absorption:
...
![](example-maze-part3_files/figure-html/unnamed-chunk-13-1.png)
...
[`survival()`](https://andrewmarx.github.io/samc/reference/survival.md)
metric. The results are very similar to part 2, but the presence of a
shortcut does increase access to both the bottom left trap and the
finish point. The relative role of each isn’t clear, but it does overall
seem to decrease the time to absorption.
...
The presence of both the traps and the shortcut has a cumulative effect
on both metrics; the time to absorption decreases, as does the time to
conditional first passage (the time to finish assuming the individual is
not absorbed elsewhere).
...
metric shows how introducing the shortcut affects the probability of
finishing the maze vs being absorbed in the traps:
...
With the introduction of the shortcut, the probability of absorption
...
the later two points are shifted by one; this is due to changing a `NA`
cell to non-`NA` for the shortcut, and it is related to why the
[`locate()`](https://andrewmarx.github.io/samc/reference/locate.md)
should be rerun when the structure of the input(s) to
[`samc()`](https://andrewmarx.github.io/samc/reference/samc.md) changes.

By now, the process of exploring the different metrics for these
...
Parts of this series may be rewritten, further expanded on, and/or
...
the construction of a maze using a graph rather than a matrix/raster.
